	...
	if (file) {
		struct inode *inode = file_inode(vma->vm_file);
               ...
               if (SUSFS_IS_INODE_OPEN_REDIRECT(inode)) {
                       ...
                       int srcu_idx = srcu_read_lock(&susfs_srcu_open_redi>
                       int ret = susfs_open_redirect_spoof_show_map_vma_sr>
                       if (!ret) {
                               pgoff = ((loff_t)vma->vm_pgoff) << PAGE_SHI>
                               start = vma->vm_start;
                               end = vma->vm_end;
                       ...
                       srcu_read_unlock(&susfs_srcu_open_redirect, srcu_id>
               }
#endif // #ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
